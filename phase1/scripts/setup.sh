...
# Note: Project requires Node.js 16+
...
# 5. Install Nginx (Reverse Proxy) & PM2 (Process Manager)
echo "--- 5. Installing Nginx and PM2 ---"
sudo apt-get install -y nginx
sudo npm install pm2@latest -g

# 6. Create application directory structures
# Application requires public/uploads directory for product images
echo "--- 6. Creating application directory structures ---"
sudo mkdir -p /var/www/app/logs
sudo mkdir -p /var/www/app/public/uploads

# Assign permissions to avoid Permission Denied errors when uploading files
sudo chown -R $USER:$USER /var/www/app

# 7. Verification for Evidence
